...
class InitMemRefSubviewMemorySpace(RewritePattern):
    ...
    def match_and_rewrite(self, op: memref.Subview, rewriter: PatternRewriter):
        ...
        # subview's result should have the same memory space 
        # as the memref the subview is viewing
        if ( op.result.type.memory_space == inputType.memory_space):
            # nothing left to do
            return
        if (not inputType.memory_space):
            # if the input memref does not have a memory space assigned,
            # assign it the default value
            inputType.memory_space = builtin.IntegerAttr(0, builtin.i32)

        # use from_static_parameters to 
        # 1) construct a new subview operation 
        # 2) set this subview's resulting memref's memspace 
        #    to the same value as the input memref's memspace
